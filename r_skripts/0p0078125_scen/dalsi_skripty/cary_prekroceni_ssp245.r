...
# === Cesty ===
output_dir <- "output/cary_prekroceni_ssp245"
...
# === Seznam profilů a období ===
cilove_profily <- c(463000, 469000, 480500)
obdobi_vec <- c("2030", "2050", "2070", "2085")
ssp_variant <- "SSP245"
...
for (profil in cilove_profily) {
  ...
  grafy <- map(obdobi_vec, function(obdobi) {
    data_sim <- fdc_sim %>%
      filter(kodprofilu == profil, obdobi == !!obdobi, scenar_short == ssp_variant)
    ...
    ggplot() +
      ...
      labs(title = paste("Období", obdobi), x = "Pravděpodobnost překročení [%]", y = "Průtok [m³/s]") +
  ...
  vystup <- (grafy[[1]] + grafy[[2]]) / (grafy[[3]] + grafy[[4]]) +
    plot_annotation(title = paste0(nazev_profilu, " – SSP245"), theme = theme(
      plot.title = element_text(size = 12, family = "Calibri", face = "bold", hjust = 0.5)
    ))

  ggsave(file.path(output_dir, paste0("FDC_SSP245_", profil, ".png")),
         plot = vystup, width = 5.2, height = 5.2, dpi = 300, bg = "white")
}

message("✅ Výstupy pro SSP245 napříč obdobími byly úspěšně vytvořeny.")
