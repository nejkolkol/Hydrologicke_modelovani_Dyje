...
obdobi_vec <- c("ONDJFM", "AMJJAS")
nazvy_map <- c("ONDJFM (zima)", "AMJJAS (léto)")
...
generuj_mapu <- function(obdobi, nazev_popisek, slozka, r_format) {
...
# === Generování map ===
map_list <- map2(obdobi_vec, nazvy_map, ~generuj_mapu(.x, .y, slozka, rozliseni_format))
...
# === Uložení ===
ggsave(
  filename = file.path(base_dir, "output_graphs", "KGE_mesicni_ondjfm_amjjas_srovnani.png"),
  plot = final_plot, width = 5.2, height = 3.2, dpi = 300, bg = "white"
)
